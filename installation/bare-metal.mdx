...
<Steps>
  ...
  <Step title="Install all dependencies">
    ...
    The `setup` script also copies the example `.env` files into place and runs the initial Prisma database generation and migration. After it completes, you will have:

    - `server/.env.development` (copied from `server/.env.example`)
