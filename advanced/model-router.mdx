---
title: "Dynamic Model Router: Auto-Select the Best LLM"
sidebarTitle: "Model Router"
description: "AnythingLLM's Model Router automatically routes each chat to the optimal LLM provider and model based on rules you define—balancing cost, speed, and capability."
---
...
<Note>
  The Model Router is a **multi-user / Docker** feature. You must have at least **two distinct LLM providers** configured in System Settings before creating a router. The router cannot route between models on the same provider alone—it needs a second provider to route to.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Time-Based Routing" icon="clock">
    Send after-hours traffic to a local Ollama model to avoid cloud API costs during low-priority windows, then route back to a cloud model during business hours.
  </Card>
...
| `currentHour` | number | The current UTC hour (0–23) |
...
|---|---|---|
| `contains` | string | Prompt contains any of the comma-separated keywords (case-insensitive) |
| `matches` | string | Prompt matches a regex pattern (e.g., `/code\|function\|bug/i`) |
